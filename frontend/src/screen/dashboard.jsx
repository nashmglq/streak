    ...
    <div className="container mx-auto px-4 sm:px-6 md:px-10 lg:px-20 min-h-screen py-8 md:py-12">
      {message && Array.isArray(message) && message.length > 0 ? (
        <div className="flex flex-wrap justify-center sm:justify-start">
          {message.map((streaks) => (
            <div
              className="flex flex-col bg-white items-center border-2 rounded-lg w-full sm:w-1/2 md:w-1/3 lg:w-1/4 xl:w-1/6 p-4 text-lg shadow-md m-2 sm:m-4 hover:scale-105 transition-all duration-300"
              key={streaks.streakId}
            >
